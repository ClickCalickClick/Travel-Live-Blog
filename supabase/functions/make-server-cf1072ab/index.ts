...
function verifyAdmin(c: any): Response | null {
  ...
  if (!envPassword) {
    ...
    return c.json({ error: "Server configuration error: ADMIN_PASSWORD secret is missing", type: "CONFIG_ERROR" }, 500);
  }

  if (!adminPassword) {
    console.error("Missing x-admin-password header");
    return c.json({ error: "Unauthorized: Missing password header", type: "MISSING_HEADER" }, 401);
  }

  // Trim whitespace to handle copy-paste issues
  const cleanAdmin = adminPassword.trim();
  const cleanEnv = envPassword.trim();

  if (cleanAdmin !== cleanEnv) {
    console.error(`Password mismatch. Received len: ${cleanAdmin.length}, Expected len: ${cleanEnv.length}`);
    return c.json({
      error: "Unauthorized: Invalid admin password",
      type: "PASSWORD_MISMATCH",
      debug: {
        receivedLen: cleanAdmin.length,
        expectedLen: cleanEnv.length
      }
    }, 401);
  }
  return null;
...
app.post("/make-server-cf1072ab/verify", async (c) => {
  const error = verifyAdmin(c);
...
app.post("/make-server-cf1072ab/kid-post", async (c) => {
  // Kid posts typically don't need admin auth if they come from "home", 
  // but if we want to restrict it, we can. The user only asked for "admin" creds to be secure.
  // Assuming Kid Message Modal doesn't require the admin password.
  try {
    const formData = await c.req.formData();
